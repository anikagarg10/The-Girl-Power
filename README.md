...
app.use((req, res) => {
    match({
        ...
        if (err) {
            return res.status(500).end(errorPageage');
        }
...
If there's an errorPage in matching we return 500 status code, if no matches are found we return 404 status code. If a match is found then, we need to create a new Redux Store instance.
